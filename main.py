...
def run_enhanced_simulation():
    ...
    print("\nOK: Data saved to data/processed/")
...
def run_tests():
    ...
    print(f"Cost savings: INR {metrics['estimated_cost_savings_inr']:.0f}")
    print("PASSED\n")
...
def run_enhanced_tests():
    ...
    print(f"  OK Generated {len(data)} rows with enhanced features")
    print(f"  OK Generated {len(ocpp_sessions)} OCPP sessions")
    print(f"  OK Generated {len(dtrs)} DTR specifications")
    
    # Test K-anonymity
    masked = apply_k_anonymity(data)
    assert len(masked) == len(data), "K-anonymity changed row count"
    print(f"  OK K-anonymity masking applied")
    print("  PASSED\n")
    ...
    print(f"  OK Peak reduction: {metrics['peak_reduction_pct']:.1f}%")
    print(f"  OK Iterations: {metrics.get('lagrangian_iterations', 'n/a')}")
    print(f"  OK Shadow prices computed: {len(shadow_df)} entries")
    print("  PASSED\n")
    ...
    print(f"  OK KDE fitted with {len(kde.arrival_data)} samples")
    print(f"  OK Peak arrival at {peaks[0][0]:.1f}:00 ({peaks[0][1]:.2f}/hr)")
    print(f"  OK Added arrival and neighbor features")
    print("  PASSED\n")
    ...
    print(f"  OK Forecast evaluated: MAE {metrics.mae:.2f} kW")
    print(f"  OK Drift detection: {drift.drift_type}")
...
def run_competition_demo():
    """Run a compact end-to-end benchmark for the redesigned system."""
    from src.evaluation.benchmarks import run_competition_benchmark

    print("=" * 60)
    print("Running Vidyut Prajna Competition Benchmark")
    print("=" * 60)
    result = run_competition_benchmark(
        max_cells=int(os.getenv("BENCHMARK_MAX_CELLS", "14")),
        num_days=int(os.getenv("BENCHMARK_NUM_DAYS", "4")),
        horizon_steps=int(os.getenv("FORECAST_STEPS", "12")),
        fast=os.getenv("BENCHMARK_FAST", "true").lower() == "true",
    )

    print("\n=== Forecast Metrics ===")
    for name, metrics in result["forecast_metrics"].items():
        print(
            f"  {name}: MAE {metrics['mae_kw']:.2f} kW, "
            f"RMSE {metrics['rmse_kw']:.2f} kW, sMAPE {metrics['smape_pct']:.1f}%"
        )

    print("\n=== Optimization Metrics ===")
    for name, metrics in result["optimization_metrics"].items():
        print(
            f"  {name}: peak {metrics.get('optimized_peak_kw', 0):.1f} kW, "
            f"PAR {metrics.get('optimized_par', 0):.2f}, "
            f"overloads {metrics.get('overload_events_after', 0)}"
        )

    print("\n=== Siting ===")
    siting = result["siting_summary"]
    print(
        f"  Portfolio method: {siting.get('portfolio_method')}, "
        f"capture improvement {siting.get('capture_improvement_pct', 0):.1f}%"
    )
    top_sites = result["top_sites"][:3]
    for site in top_sites:
        print(
            f"  #{site['rank']} {site.get('zone_name', 'Unknown')}: "
            f"score {site.get('siting_score', 0):.1f}, "
            f"{site.get('capacity_feasibility', 'n/a')}"
        )
...
def main():
    ...
    parser.add_argument(
        "--competition-demo", action="store_true",
        help="Run the competition-grade benchmark pipeline"
    )
