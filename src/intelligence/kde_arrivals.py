...
class KDEConfig:
    ...
    random_seed: int = 42                   # Deterministic jitter for aggregate fitting
...
class SpatioTemporalKDE:
    ...
    def estimate_arrival_rate(
        ...
        for _, arrival in relevant.iterrows():
            ...
            # Spatial component is dimensionless; the temporal kernel already
            # supplies the events/hour unit.
            if arrival["h3_cell"] == target_cell:
                spatial_weight = 1.0
            else:
                distance = self._h3_distance_km(arrival["h3_cell"], target_cell)
                if self.kernel_name == "gaussian":
                    spatial_weight = math.exp(-0.5 * (distance / self.config.spatial_bandwidth_km) ** 2)
                else:
                    u = distance / self.config.spatial_bandwidth_km
                    spatial_weight = max(0.0, 1.0 - u ** 2) if abs(u) <= 1 else 0.0
            
            density += temporal_weight * spatial_weight
        
        exposure_days = max(1, pd.to_datetime(relevant["timestamp"]).dt.normalize().nunique())
        return float(density / exposure_days)
