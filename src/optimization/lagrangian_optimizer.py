...
AllocationValue = Union[int, Dict[int, float]]
Allocation = Dict[str, AllocationValue]
...
class LagrangianOptimizer:
    ...
    def _allocation_items(
        ...
        """Yield (time_idx, load) pairs for an allocation value."""
        value = allocation.get(task.task_id)
        if value is None:
            yield task.original_time_idx, task.energy_kwh
        elif isinstance(value, dict):
            for time_idx, amount in value.items():
                if amount > 1e-9:
                    yield int(time_idx), float(amount)
        else:
            yield int(value), task.energy_kwh

    def _delivered_before_deadline(
        self,
        task: ChargingTask,
        allocation: Allocation,
    ) -> float:
        return sum(
            amount
            for time_idx, amount in self._allocation_items(task, allocation)
            if time_idx <= task.deadline_time_idx
        )
    ...
    def _compute_load_profile(
        ...
        for task in tasks:
            for assigned_t, amount in self._allocation_items(task, allocation):
                if 0 <= assigned_t < n_times:
                    load_by_time[assigned_t] += amount
    ...
    def _compute_primal_objective(
        ...
        # SoC deficit term. A task has no service deficit when all of its
        # requested energy is delivered before its deadline.
        soc_term = 0.0
        for task in tasks:
            delivered = self._delivered_before_deadline(task, allocation)
            soc_deficit = max(0.0, 1.0 - delivered / max(task.energy_kwh, 1e-6))
            soc_term += self.beta * task.priority_weight * soc_deficit * task.energy_kwh
        ...
        for task in tasks:
            for assigned_t, amount in self._allocation_items(task, allocation):
                ...
                if mask.any():
                    ...
                    solar_bonus = min(solar / max(amount, 1), 1.0)
                    tariff_cost += self.gamma_tariff * tariff * amount
                    tariff_cost -= self.gamma_solar * solar_bonus * amount
    ...
    def _solve_subproblem(
        ...
        allocation: Allocation = self._solve_divisible_mcdm(tasks, multipliers, key_order, df)
        return allocation

    def _solve_divisible_mcdm(
        self,
        tasks: List[ChargingTask],
        multipliers: np.ndarray,
        key_order: List[Tuple[str, int]],
        ...
        """Solve a divisible deadline-constrained MCDM scheduling subproblem."""
        allocation: Allocation = {}
        unique_times = sorted(df["timestamp"].unique())
        n_times = len(unique_times)
        mult_lookup = {key: float(mult) for key, mult in zip(key_order, multipliers)}

        indexed = df.set_index(["h3_cell", "time_idx"], drop=False)
        base_by_time = df.groupby("time_idx")["grid_base_load_kw"].sum().to_dict()
        demand_col = "_optimizer_demand_kw"
        if demand_col not in df.columns:
            demand_col = "baseline_ev_load_kw" if "baseline_ev_load_kw" in df.columns else ""
        if demand_col:
            baseline_by_time = (
                (df["grid_base_load_kw"] + df[demand_col])
                .groupby(df["time_idx"])
                .sum()
                .astype(float)
                .to_dict()
            ...
            baseline_by_time = {t: float(load) for t, load in base_by_time.items()}
            baseline_cell_peak = {}

        target_avg = float(np.mean(list(baseline_by_time.values()))) if baseline_by_time else 0.0
        baseline_peak = max(baseline_by_time.values()) if baseline_by_time else float("inf")
        agg_load = {t: float(base_by_time.get(t, 0.0)) for t in range(n_times)}
        cell_total = {
            (str(row["h3_cell"]), int(row["time_idx"])): float(row["grid_base_load_kw"])
            for _, row in df.iterrows()
        }
        optimized_ev = {(cell, t): 0.0 for cell, t in cell_total}

        for task in [task for task in tasks if not task.is_shiftable]:
            time_idx = task.original_time_idx
            allocation[task.task_id] = {time_idx: task.energy_kwh}
            key = (task.h3_cell, time_idx)
            if key in cell_total:
                cell_total[key] += task.energy_kwh
                optimized_ev[key] += task.energy_kwh
                agg_load[time_idx] += task.energy_kwh

        def row_for(cell: str, time_idx: int) -> Optional[pd.Series]:
            try:
                value = indexed.loc[(cell, time_idx)]
            except KeyError:
                return None
        ...
        def capacity_for(row: pd.Series) -> float:
            return derated_capacity(
                float(row["transformer_capacity_kw"]),
                float(row.get("temperature_c", 30)),
            ) * self.max_utilization

        def candidate_score(task: ChargingTask, row: pd.Series, time_idx: int, amount: float) -> float:
            key = (task.h3_cell, time_idx)
            before_agg = agg_load.get(time_idx, 0.0)
            projected_agg = before_agg + amount
            variance_delta = (projected_agg - target_avg) ** 2 - (before_agg - target_avg) ** 2
            tariff = float(row["tariff_multiplier"])
            solar_available = max(0.0, float(row.get("solar_generation_kw", 0.0)) - optimized_ev.get(key, 0.0))
            solar_bonus = min(solar_available / max(amount, 1e-6), 1.0)
            delay_span = max(1, task.deadline_time_idx - task.original_time_idx)
            delay_ratio = max(0, time_idx - task.original_time_idx) / delay_span
            utilization = (cell_total.get(key, 0.0) + amount) / max(capacity_for(row), 1e-6)
            overload_barrier = 0.0 if utilization <= 1.0 else 1_000_000.0 * (utilization - 1.0) ** 2
            peak_barrier = 0.0
            if projected_agg > baseline_peak:
                peak_barrier = 10_000.0 * (projected_agg - baseline_peak) ** 2
            cell_peak = baseline_cell_peak.get(task.h3_cell, float("inf"))
            if cell_total.get(key, 0.0) + amount > cell_peak:
                peak_barrier += 10_000.0 * (cell_total.get(key, 0.0) + amount - cell_peak) ** 2
            return (
                self.alpha * variance_delta
                + self.beta * task.priority_weight * delay_ratio * amount
                + self.gamma_tariff * tariff * amount
                - self.gamma_solar * solar_bonus * amount
                + mult_lookup.get(key, 0.0) * amount
                + overload_barrier
                + peak_barrier
            )

        def available_at(task: ChargingTask, time_idx: int, row: pd.Series, assigned: Dict[int, float]) -> float:
            key = (task.h3_cell, time_idx)
            local_limit = min(capacity_for(row), baseline_cell_peak.get(task.h3_cell, float("inf")))
            cap_room = local_limit - cell_total.get(key, 0.0)
            task_room = task.max_power_kw - assigned.get(time_idx, 0.0)
            return max(0.0, min(cap_room, task_room))

        flexible_tasks = [task for task in tasks if task.is_shiftable]
        flexible_tasks.sort(
            key=lambda task: (
                task.deadline_time_idx - task.original_time_idx,
                -task.energy_kwh,
        ...
        for task in flexible_tasks:
            ...
            remaining = float(task.energy_kwh)
            chunk_size = max(0.5, min(task.energy_kwh / 4.0, task.max_power_kw / 2.0))
            slots = range(max(0, task.original_time_idx), min(task.deadline_time_idx, n_times - 1) + 1)
            while remaining > 1e-6:
                amount_goal = min(chunk_size, remaining)
                best: Optional[Tuple[float, int, float]] = None
                for time_idx in slots:
                    row = row_for(task.h3_cell, time_idx)
                    if row is None:
                        continue
                    available = available_at(task, time_idx, row, assigned)
                    if available <= 1e-6:
                        continue
                    amount = min(amount_goal, available, remaining)
                    score = candidate_score(task, row, time_idx, amount)
                    if best is None or score < best[0]:
                        best = (score, time_idx, amount)
                if best is None:
                    fallback: Optional[Tuple[float, int, float]] = None
                    for time_idx in slots:
                        row = row_for(task.h3_cell, time_idx)
                        if row is None:
                            continue
                        amount = min(amount_goal, remaining, task.max_power_kw - assigned.get(time_idx, 0.0))
                        if amount <= 1e-6:
                            continue
                        score = candidate_score(task, row, time_idx, amount)
                        if fallback is None or score < fallback[0]:
                            fallback = (score, time_idx, amount)
                    if fallback is None:
                        break
                    best = fallback
                _, time_idx, amount = best
                key = (task.h3_cell, time_idx)
                assigned[time_idx] = assigned.get(time_idx, 0.0) + amount
                cell_total[key] = cell_total.get(key, 0.0) + amount
                optimized_ev[key] = optimized_ev.get(key, 0.0) + amount
                agg_load[time_idx] = agg_load.get(time_idx, 0.0) + amount
                remaining -= amount
            if remaining > 1e-5:
                time_idx = min(task.deadline_time_idx, n_times - 1)
                assigned[time_idx] = assigned.get(time_idx, 0.0) + remaining
            allocation[task.task_id] = assigned
        return allocation
    ...
    def _find_minimum_regret(
        self,
        pareto_front: List[Tuple[float, float, Allocation]],
    ) -> Allocation:
        """
    ...
    def optimize(
        ...
        pareto_front: List[Tuple[float, float, Allocation]] = []
        best_allocation: Allocation = {task.task_id: {task.original_time_idx: task.energy_kwh} for task in tasks}
        ...
        for iteration in range(self.max_iterations):
            ...
            soc_deficit = sum(
                task.priority_weight
                * max(0.0, 1.0 - self._delivered_before_deadline(task, allocation) / max(task.energy_kwh, 1e-6))
                for task in tasks
            )
        ...
        if (
            ...
            or metrics["variance_reduction_pct"] < -1e-6
            or metrics["overload_events_after"] > metrics["overload_events_before"]
        ):
            df = self._apply_baseline_schedule(df, demand_col)
            metrics = self._compute_metrics(df, demand_col)
            metrics["fallback_reason"] = "baseline_guardrail_no_worse_solution"
    ...
    def _apply_baseline_schedule(self, df: pd.DataFrame, demand_col: str) -> pd.DataFrame:
        """Use the unmanaged schedule when shifting cannot improve safely."""
        df = df.copy()
        df["baseline_ev_load_kw"] = df[demand_col]
        df["optimized_ev_load_kw"] = df[demand_col]
        ...
        if "effective_capacity_kw" not in df.columns:
        ...
        df.drop(columns=["_optimizer_demand_kw"], inplace=True, errors="ignore")
        return df
    ...
    def _apply_allocation(
        ...
        for task in tasks:
            for assigned_t, amount in self._allocation_items(task, allocation):
                key = (task.h3_cell, assigned_t)
                cell_time_optimized[key] = cell_time_optimized.get(key, 0.0) + amount
